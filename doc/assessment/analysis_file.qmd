...
IMDb is ruled out for the following reasons:

- No official public API available: access is provided via AWS with a minimum entry cost of **$150,000**.
- Incompatible with the project's budget and technical requirements.

The evaluation therefore focuses solely on **OMDb** and **TMDB**.

## JSON Comparison — OMDb vs TMDB

Comparison based on the same search query (*"The Odyssey"*), showing the relevant data returned by each API:

| Data Field | OMDb (JSON) | TMDB (JSON, `language=fr-FR`) |
|---|---|---|
| **Title** | `"Title": "The Odyssey"` (English only) | `"title": "L'Odyssée"` (translated into French) |
| **Synopsis** | `"Plot"` in English only | `"overview"` translated into French |
| **Genres** | `"Genre": "Action, Adventure, Fantasy"` (English text) | `"genres": [{"id_genre": "Aventure"}, ...]` (translated) |
| **Runtime** | `"Runtime": "172 min"` | `"runtime": 172` (minutes) |
| **Release Date** | `"Released": "17 Jul 2026"` | `"release_date": "2026-07-15"` |
| **Content Rating** | `"Rated": "R"`: US age rating classification | `"adult": false`: indicates if the movie is rated adult/pornographic<br>`"softcore": false`: specific flag designating non-explicit erotic content |
...
{
  "Title": "The Odyssey",
  "Plot": "A retelling of Homer's epic tale of Odysseus's journey home.",
...
{
  "title": "L'Odyssée",
  "overview": "Le retour périlleux d'Ulysse après la guerre de Troie.",
  "genres": [
    { "id_genre": "Aventure" },
    { "id_genre": "Action" },
    { "id_genre": "Fantastique" }
  ],
  "runtime": 172,
...
**TMDB is selected** as the reference API for the project.

It is the only one of the two candidates to return key fields — title, synopsis, genres — **translated into French** via the `language=fr-FR` parameter, whereas OMDb strictly provides these fields in English.
